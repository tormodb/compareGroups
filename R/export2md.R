export2md<-function(x, which.table="descr", nmax=TRUE, header.labels=c(), caption=NULL, ...){
  ...
  if (ww %in% c(1)) {  
    ...
    return(knitr::kable(table1, align = align, row.names = FALSE, caption=caption[1], booktabs = TRUE))
  }      
  if (ww %in% c(2)){
    ...
    return(knitr::kable(table2, align = align, row.names = FALSE, caption=caption[2], booktabs = TRUE))
  }    
}
